...
final class MoodEngine: ObservableObject {
    ...
    static func title(for m: MoodVector, topGenre: String?) -> String {
        if m.bass > 0.8 { return "High-Bass Head" }
        if m.organic > 0.7 && m.energy < 0.45 { return "Zenned-Out Hippie" }
        if m.energy > 0.75 && m.valence > 0.65 { return "Sunlit Maximalist" }
        if m.energy > 0.7 && m.valence < 0.5 { return "Midnight Driver" }
        if m.energy < 0.4 && m.valence > 0.6 { return "Mellow Optimist" }
        if m.energy < 0.4 && m.valence < 0.5 { return "Deep Introspective" }
        if m.organic > 0.65 { return "Acoustic Soul" }
        if m.valence > 0.7 { return "Feel-Good Seeker" }
